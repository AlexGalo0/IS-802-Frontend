...
export const AdminDenuncias = () => {
	...
	const mutationDesestimar = useMutation({
		mutationFn: (idDesestimar) => desestimarDenuncia(idDesestimar),
	...
	const mutationDarBaja = useMutation({
		mutationFn: (idDenuncia) => darBajaDenunciaUsuario(idDenuncia),
		onSuccess: () => {
			queryClient.invalidateQueries("denuncias");
				...
				<header className='headerCrud' /* style={{ paddingTop: "122px" }} */>
					...
					>
						...
						<Container className='conCrud'>
							<div
								className='container-table' /* style={{border: '1px solid black'}} */
							>
								<table>
									<tr
										style={{
											...
											>
												...
												>
													<button
														onClick={() => {
															handleDesestimar(
																denuncia.id_denuncia.data.toString()
															);
														}}
